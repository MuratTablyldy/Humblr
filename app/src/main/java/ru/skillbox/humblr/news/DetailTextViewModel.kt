...
@HiltViewModel
class DetailTextViewModel @Inject constructor(private val repository: MainRepository) :
    ViewModel() {
    ...
    val more: LiveData<More?>
        get() = _more
    private val _link = MutableLiveData<Link.LinkText>(null)
    val link: LiveData<Link.LinkText>
        get() = _link
        ...
        viewModelScope.launch {
            ...
            when (value) {
                is Result.Success -> {
                    ...
                    val linkq = array[0].data.children?.first()?.data as Link.LinkText
