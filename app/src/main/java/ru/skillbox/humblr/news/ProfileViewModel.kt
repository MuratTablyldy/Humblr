...
class ProfileViewModel @Inject constructor(val repository: MainRepository): ViewModel() {
    ...
    fun getSubreddits(before:String?,after:String?,count:Int?,limit:Int?){
        viewModelScope.launch {
            when(val subreddits=repository.getMineSubreddits(before = before, after = after, count = count, limit = limit,true)){
                is Result.Success->{
    ...
    suspend fun getCommentsSaved(after: String?,
                                 before: String?,
                                 count: Int?,
                                 limit: Int?,
                                 srDetail:Boolean?,
                                 ...
                                 context:Int,
                                 sort: RedditApi.Sort
    )=repository.getCommentsSaved(account.value?.name!!, after, before, count, limit, srDetail, time, context, sort)
    ...
    suspend fun getSavedSubreddit(
        ...
        context:Int,
        sort: RedditApi.Sort
    )=repository.getSubredditSaved(account.value?.name!!, after, before, count, limit, srDetail, time, context, sort)
    fun exit(){
