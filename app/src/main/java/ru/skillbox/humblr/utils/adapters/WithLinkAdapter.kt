...
class WithLinkAdapter(private val listener: MListener) :
    ...
    override fun onBindViewHolder(
        ...
        holder.binding?.join?.onClickListener {
            item.subInfo?.name?.let { it1 -> listener.onJoin(holder.binding!!.join, it1,holder.binding!!.title) }
        }
        if(item.subInfo!=null){
            if(item.subInfo!!.userIsSubscriber == true){
                holder.binding!!.title.setColor(holder.itemView.context.resources.getColor(R.color.selected,null))
