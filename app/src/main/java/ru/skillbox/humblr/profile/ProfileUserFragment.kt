...
import ru.skillbox.humblr.news.RecycleViewModel
...
@AndroidEntryPoint
class ProfileUserFragment : Fragment(), LCEERecyclerView2.OnLoad, MListener,CallBack {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = NewsAdapter(listener = this, this)
        ...
        viewModel.apply {
            ...
            errors.observe(viewLifecycleOwner) {
                if(it is MainRepository.TokenISInvalidException){
                    (activity as MainActivity).onTokenExpired(this@ProfileUserFragment)
                    return@observe
                } else{
    ...
    override fun onYoutube(link: String, id: String, second: Long) {
        (activity as MainActivity).navigateToYoutubeFragment(link = link, time = second, id = id)
    }

    override fun invoke() {
        viewModel.errors.postValue(null)
        viewModel.state.postValue(RecycleViewModel.State.INIT)
    }
